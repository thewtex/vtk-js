...
import ComputeGradientsWorker from './ComputeGradients.worker.js';
...
function vtkOpenGLTexture(publicAPI, model) {
  ...
  publicAPI.create3DLighting = function(scalarTexture, data, spacing) {
    ...
    const gradientsWorker = new Float32Array(width * height * depth * 4);
    const gradientMagnitudesWorker = new Float32Array(width * height * depth);
    ...
    while(depthIndex < depth ) {
      const worker = new WebworkerPromise(new ComputeGradientsWorker());
      workers.push(worker.postMessage({ width, height, depth, spacing, data, haveWebgl2, depthStart: depthIndex, depthEnd: Math.min(depthIndex + depthStride, depth - 1) }, []));
      depthIndex += depthStride;
    }
    Promise.all(workers).then(function(workerResults) {
      const gradients = new Float32Array(width * height * depth * 4);
      const gradientMagnitudes = new Float32Array(width * height * depth);
      let minMag = Infinity;
      let maxMag = -Infinity;

      workerResults.forEach(function({ subGradients, subGradientMagnitudes, subMinMag, subMaxMag, subReformattedGradients, subDepthStart }) {
        const start = subDepthStart * width * height;
        gradients.set(subGradients, start * 4)
        gradientMagnitudes.set(subGradientMagnitudes, start);
      ...
      if (haveWebgl2) {
        ...
        for (let p = 0; p < numPixelsIn; ++p) {
          ...
          reformattedGradients[outIdx++] = 255.0 * Math.sqrt(gradients[pp + 3] / maxMag);
        }
      }
        ...
        for (let inY = 0; inY < height; inY += vinfo.ystride) {
          for (let xRep = 0; xRep < xrepsThisRow; xRep++) {
            ...
            for (let inX = 0; inX < width; inX += vinfo.xstride) {
              // copy value
              reformattedGradients[outIdx++] = 127.5 + 127.5 * gradients[inOffset + inX * 4];
